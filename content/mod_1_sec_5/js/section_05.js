...
var s05q01_answer = new Audio('audio/s05q01_answer.mp3');
var s05q01_question = new Audio('audio/s05q01_question.mp3');
var s05q01_warden = new Audio('audio/s05q01_warden.mp3');
var s05q02_legal = new Audio('audio/s05q02_legal.mp3');
 ...
 */
function load() {
  m_data.load("mod_1_sec_5.json", load_cb, function(percent, load_time) {
...
function load_cb(data_id) {
  ...
  charSue = m_scenes.get_object_by_name("Sue_Basic");
  charKeani = m_scenes.get_object_by_name("Keani_Basic");
  charAnwar = m_scenes.get_object_by_name("Anwar_Basic");
  charLi = m_scenes.get_object_by_name("Li_Basic");
  charLi2 = m_scenes.get_object_by_name("Li_Basic.001");

  m_animation.apply(charTom, "Tom_Assembly_Idle_B4W_BAKED");
  m_animation.play(charTom);
  m_animation.set_behavior(charTom, m_animation.AB_CYCLIC);
  m_animation.apply(charSue, "Sue_Warden_Announce_B4W_BAKED");
  m_animation.play(charSue);
  m_animation.set_behavior(charSue, m_animation.AB_CYCLIC);
  m_animation.apply(charKeani, "Keani_Assembly_Idle_B4W_BAKED");
  m_animation.play(charKeani);
  m_animation.set_behavior(charKeani, m_animation.AB_CYCLIC);
  m_animation.apply(charAnwar, "Anwar_Assembly_Idle_B4W_BAKED");
  m_animation.play(charAnwar);
  m_animation.set_behavior(charAnwar, m_animation.AB_CYCLIC);
  m_animation.apply(charLi, "Li_Assembly_Idle_B4W_BAKED");
  m_animation.play(charLi);
  m_animation.set_behavior(charLi, m_animation.AB_CYCLIC);
  m_animation.apply(charLi2, "Li_Assembly_Idle_B4W_BAKED");
  m_animation.play(charLi2);
  m_animation.set_behavior(charLi2, m_animation.AB_CYCLIC);

  m_scene_mouse.set_hover_cursor(m_scenes.get_object_by_name("Mesh_Sue"), "pointer");
  m_scene_mouse.set_hover_cursor(m_scenes.get_object_by_name("Mesh_Anwar"), "pointer");
  m_scene_mouse.set_hover_cursor(m_scenes.get_object_by_name("Mesh_Li"), "pointer");
  m_scene_mouse.set_hover_cursor(m_scenes.get_object_by_name("Mesh_Li.001"), "pointer");
  m_scene_mouse.set_hover_cursor(m_scenes.get_object_by_name("Mesh_Keani"), "pointer");
  m_scene_mouse.set_hover_cursor(m_scenes.get_object_by_name("Assembly_Tom_Car"), "pointer");
  m_scene_mouse.set_hover_cursor(m_scenes.get_object_by_name("Assembly_Sign"), "pointer");
  m_scene_mouse.set_hover_cursor(m_scenes.get_object_by_name("Assembly_Parts.001"), "pointer");
  m_scene_mouse.set_hover_cursor(m_scenes.get_object_by_name("Assembly_Parts.002"), "pointer");
...
function play_cutscene(buffer) {
  ...
  m_cutscene.play_cutscene(container, ["video/section_05.mp4"], play_disclamer, buffer);//Set Outro here
}

function play_disclamer() {
  var legal = document.getElementById("legal");
  legal.style.display = "block";
  s05q02_legal.addEventListener("ended", end_cutscene);
  s05q02_legal.play();
}
...
function on_mouse_down(e) {
  dragObj = null;
  if (m_scenes.pick_object(e.clientX, e.clientY) != null){
  ...
  m_quiz.add_question({
    ...
    check: function() {
      ...
      if (validationData[0] == false) {
        ...
        //==============================================
        GiveHint("<strong>This isn’t safe. The fire warden will need to make sure everyone is accounted for. Try again.</strong>",5000,false);
        //==============================================
        $('.balance-text').balanceText();
        switch (validationData[1]){
          case "Mesh_Keani":
          case "Mesh_Li.001":
          case "Assembly_Parts.002":
            m_animation.apply(charTom,"Tom_Assembly_NoSue_B4W_BAKED");
            m_animation.set_behavior(charTom, m_animation.AB_FINISH_STOP);
            m_animation.play(charTom, setIdle());
            m_notifications.setIcon(2,"cross",null,null);
          break;
          case "Assembly_Tom_Car":
            m_animation.apply(charTom,"Tom_Assembly_NoCar_B4W_BAKED");
            m_animation.set_behavior(charTom, m_animation.AB_FINISH_STOP);
            m_animation.play(charTom, setIdle());
        ...
        question_text.innerHTML =
        //==============================================
        "<strong>Yes. You need to go to your emergency assembly area and wait until the emergency warden gives the all clear.</strong>";
        //==============================================
        $('.balance-text').balanceText();
        m_animation.apply(charTom,"Tom_Assembly_Walk_B4W_BAKED");
        m_animation.set_behavior(charTom, m_animation.AB_FINISH_STOP);
        m_animation.play(charTom);
...
//******************************
  ...
  function GetHint(){
    switch (objPar){
    case "noti_icons_0":
      GiveHint("<strong>Drive away from the danger.</strong>",1000,true);
      break;
    case "noti_icons_1":
      GiveHint("<strong>Assemble at the marked assembly point.</strong>",1250,true);
      break;
    case "noti_icons_2":
      GiveHint("<strong>Stand with your co-workers</strong>",1000,true);
      break;
    }
  }
